...
def postprocess(outputs, img_shape, conf_thres=0.25, iou_thres=0.45):
    ...
    outputs = np.transpose(np.squeeze(outputs))  # ת�ò�ѹ�����
    rows = outputs.shape[0]
    boxes = []
    scores = []
    class_ids = []
    x_factor = img_shape[1] / input_imgW  # ����ģ������ߴ�Ϊ640x640
    y_factor = img_shape[0] / input_imgH
    for i in range(rows):
        classes_scores = outputs[i][4:]  # ��ȡ������
        ...
        if max_score >= conf_thres:
            ...
            w = w * x_factor
            h = h * y_factor
            boxes.append([x - w / 2, y - h / 2, x + w / 2, y + h / 2])  # ת��Ϊ���ϽǺ����½�����
...
def preprocess_image(image_path, input_shape=(input_imgW, input_imgH)):
    ...
    image = image.astype(np.float32) / 255.0  # ��һ��
    image = np.transpose(image, (2, 0, 1))  # ת��ΪCHW��ʽ
    image = np.expand_dims(image, axis=0)  # ���batchά��
...
if __name__ == "__main__":
    ...
    # ����Ƶ�ļ�
        ...
        out = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'XVID'), 20.0, (640, 480)) # *'mp4v' \ *'MJPG'
        while (cap.isOpened()):
            ret, frame = cap.read()
            if not ret:
                break
